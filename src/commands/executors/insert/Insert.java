...
public class Insert implements IExecutor {
    ...
    Relation insert(String[] commands, Interpreter interpreter)
        ...
        Relation rel = RelationLoader
                .loadRelation(nomRelation, interpreter.getDbPath());
...
//        Vector<?> values  = InsertValuesHandler
//                .getValuesInsert( commands );
        String[] vals = InsertValuesHandler.getStringValues(commands);
        Vector<?> values  = InsertValuesHandler
                .getValuesInsert( rel, columns, vals );
